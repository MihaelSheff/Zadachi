close all;
clear all;
clc;

%%
number_of_bits = 14;
filename = 'vrpesnia.mp3';
[data,sample_rate] = audioread(filename);
left_channel = data(:,1)*2^number_of_bits;
right_channel = data(:,2)*2^number_of_bits;
%%
load('numerator.mat');
numerator = numerator*2^number_of_bits;
filtered_left_channel = conv(left_channel,numerator);
filtered_right_channel = conv(right_channel,numerator);
filtered_data = [filtered_left_channel,filtered_right_channel]/2^(2*number_of_bits);
filtered_data = filtered_data/max(abs(filtered_data));
%%
new_filename = strrep(filename,'.mp3','_filtered_fixed_point.wav');
audiowrite(new_filename,filtered_data,sample_rate);